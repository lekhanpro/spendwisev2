    ...
    <div className="p-4 pb-24 space-y-4 animate-slide-up">
      ...
      <div className="space-y-3">
        {budgetsWithProgress.map(budget => (
          <div key={budget.id} className="bg-white dark:bg-slate-800 shadow-md rounded-2xl p-4">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl" style={{ backgroundColor: (budget.category?.color || '#ccc') + '20' }}>
                {budget.category?.icon}
              </div>
              <div className="flex-1">
                <p className="font-semibold text-slate-800 dark:text-white">{budget.category?.name}</p>
                <p className="text-sm text-slate-400 capitalize">{budget.period}</p>
              </div>
